...
// store the time. defaults to zero
int time = 0;
...
// add the three new system calls
struct logentry schedlog[LOG_SIZE];
int log_count = __INT_MAX__;
...
int sys_nice(void)
{
  ...
  // update nice value, keep in proper range
...
freeproc(struct proc *p)
{
  ...
  // set the nice value to 0 by default
  p->nice = 0;
}
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        if (log_count < LOG_SIZE && !(log_count && schedlog[log_count-1].pid == p->pid)) {
...
yield(void)
{
  ...
  acquire(&p->lock);
  p->state = RUNNABLE;
  // increment the time to know another tick has passed
  ++time;
  sched();
  release(&p->lock);
}
